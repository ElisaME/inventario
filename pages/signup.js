...
const signup = () => {
  ...
  const logInAccount = async (e) => {
    e.preventDefault()
    const res = await fetch('https://radiant-ridge-01211.herokuapp.com/signup', {
      method: 'POST',
      headers: {
        ...
        <form class="mt-8 space-y-6" onSubmit={logInAccount}>
          ...
          <div class="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input onChange={handleInput} id="name" name="name" type="text" autocomplete="name" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Nombre"/>
            </div>
            <div>
